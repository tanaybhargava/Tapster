...
public class AzureServiceConnection
{
	...
	public static <T extends TapsterNotificationHandler> void Initialize(
			Activity mainActivity, Class<T> notificationHandlerClass)
	{
	...
	private AzureServiceConnection(Activity mainActivity,
			Class<TapsterNotificationHandler> notificationHandlerClass)
	{
		...
		try
		{
			...
			mClient = new MobileServiceClient(Config.APPURL, Config.APPKEY, mainActivity).withFilter(new ProgressFilter(mainActivity)).withFilter(new RefreshTokenCacheFilter());

		} catch (MalformedURLException e)
		{
			Utility.createAndShowDialog(new Exception("Error creating the Mobile Service. " + "Verify the URL"), "Error", mainActivity);
	...
	private class RefreshTokenCacheFilter implements ServiceFilter
	{
		...
		/**
		 * The AuthenticationRetryFilterCallback class is a wrapper around the
		 * response callback that encapsulates the request and other information
		 * needed to enable a retry of the request when HTTP status code 401 is
		 * encountered.
		 */
		private class AuthenticationRetryFilterCallback implements
				ServiceFilterResponseCallback
		{
			...
			public AuthenticationRetryFilterCallback(
					...
					Exception exception)
			{
				...
				{
					// Two simultaneous requests from independent threads could
					// get HTTP
					// status 401. Protecting against that right here so
					// multiple
					// authentication requests are not setup to run on the UI
					// thread.
					// We only want to authenticate once. Other requests should
					// just wait
					...
					// Retry recursively with a new token as long as we get a
					// 401.
					mNextServiceFilterCallback.onNext(this.mRequest, this);
				}

				// Responses that do not have 401 status codes just pass
				// through.
				...
				ServiceFilterResponseCallback responseCallback)
		{
			...
			// Wrap the request in a callback object that will facilitate
			// retries.
			AuthenticationRetryFilterCallback retryCallbackObject = new AuthenticationRetryFilterCallback(request, nextServiceFilterCallback, responseCallback);
	...
	public static void LogOut(Activity activity)
	{
		...
		AzureServiceConnection.instance.getClient().logout();
		SharedPreferences prefs = activity.getSharedPreferences(Config.SHAREDPREFFILE, Context.MODE_PRIVATE);
